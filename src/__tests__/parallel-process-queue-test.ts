...
describe('ParallelProcessQueue', function() {
  it('should find all the files of a given extension inside of the directory ' +
     'that it is given to scan', function(finished) {
     fsExtra.removeSync(__dirname + '/tmp');
     fs.mkdirSync(__dirname + '/tmp');
     fs.writeFileSync(__dirname + '/tmp/test.value', '');
     const sequence:LazySequence.Sequence<File.AbsoluteFilePath> = ParallelProcessQueue.findFiles(File.getAbsoluteFilePath(__dirname + '/tmp'), 'value');
     const future:Promise.Future<File.AbsoluteFilePath[]> = LazySequence.evaluate(sequence);
     Promise.then(function(fileLocations:File.AbsoluteFilePath[]) {
       expect(fileLocations).toEqualJSON([{absolutePath:__dirname + '/tmp/test.value'}]);
  ...
  it('should find no files when the given extension has no files', function(finished) {
     ...
     fs.mkdirSync(__dirname + '/tmp');
     fs.writeFileSync(__dirname + '/tmp/test.value', '');
     const sequence:LazySequence.Sequence<File.AbsoluteFilePath> = ParallelProcessQueue.findFiles(File.getAbsoluteFilePath(__dirname + '/tmp'), 'enumValue');
     const future:Promise.Future<File.AbsoluteFilePath[]> = LazySequence.evaluate(sequence);
     Promise.then(function(fileLocations:File.AbsoluteFilePath[]) {
  ...
  it('returns a promise for multiple values and directories when given a ' +
    ...
    const future:Promise.Future<File.AbsoluteFilePath[]> = LazySequence.evaluate(sequence);
    Promise.then(function(fileLocations:File.AbsoluteFilePath[]) {
      expect(fileLocations).toContain({absolutePath:__dirname + '/tmp/test.value'});
      expect(fileLocations).toContain({absolutePath:__dirname + '/tmp/test2.value'});
      expect(fileLocations).toContain({absolutePath:__dirname + '/tmp/tmp2/test.value'});
      expect(fileLocations).toContain({absolutePath:__dirname + '/tmp/tmp3/test.value'});
      expect(fileLocations).toContain({absolutePath:__dirname + '/tmp/tmp3/tmp4/test2.value'});
  ...
  it('returns a promise for multiple values and directories when given a ' +
    ...
    'files that look like directories but are not', function(finished) {
    fsExtra.removeSync(__dirname + '/tmp');
    fs.mkdirSync(__dirname + '/tmp');
