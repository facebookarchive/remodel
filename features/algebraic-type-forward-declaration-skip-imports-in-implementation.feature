...
Feature: Outputting forward declarations in Algebraic Types
  ...
  @announce
  Scenario: Generating an algebraic type with forward declarations also skip
  imports in implementation file
    Given a file named "project/values/SimpleADT.adtValue" with:
      """
      %type name="SomeType"

      SimpleADT includes(UseForwardDeclarations, SkipImportsInImplementation) {
        FirstSubtype {
  ...
  @announce
  Scenario: Generating an algebraic type containing an enum with forward declarations also skip
  imports in implementation file
    Given a file named "project/values/SimpleADT.adtValue" with:
      """
