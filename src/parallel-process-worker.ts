...
function processFoundFileRequest(request:ParallelProcess.FindFilesRequest) {
  const foundFilesFuture:Promise.Future<Either.Either<Error.Error,FileFinder.FilesAndDirectories>> = FileFinder.findFilesAndDirectories(request.directoryToSearch, request.fileExtension);
  Promise.then(function(either:Either.Either<Error.Error, FileFinder.FilesAndDirectories>) {
    const response:ParallelProcess.Response = Either.match(function(error:Error.Error) {
