...
function toAsyncLoggingPipelineForm<T, U>(
  f: (t: T) => Promise.Future<Logging.Context<U>>,
): (
...
export function mapLoggedSequence<T, U>(
  seq: LazySequence.Sequence<Promise.Future<Logging.Context<T>>>,
  f: (t: T) => Promise.Future<Logging.Context<U>>,
): LazySequence.Sequence<Promise.Future<Logging.Context<U>>> {
  return seq.map(toAsyncLoggingPipelineForm(f));
}

export function mapLoggedSequence3<T, U, V>(
  seq: LazySequence.Sequence<Promise.Future<Logging.Context<T>>>,
  f: (t: T) => Promise.Future<Logging.Context<U>>,
  g: (u: U) => Promise.Future<Logging.Context<V>>,
): LazySequence.Sequence<Promise.Future<Logging.Context<V>>> {
  return seq
    .map(toAsyncLoggingPipelineForm(f))
    .map(toAsyncLoggingPipelineForm(g));
}

function evaluateFutureLoggingForLogger<T>(
...
export function evaluatedLoggedSequence<T>(
  logger: Logging.Logger,
  seq: LazySequence.Sequence<Promise.Future<Logging.Context<T>>>,
): LazySequence.Sequence<Promise.Future<Logging.Context<T>>> {
