...
describe('Map', function() {
  describe('#get', function() {
    ...
    it('should return the actual value when looking up on a map that has ' +
       'that key', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
      const actualValue = Map.get('key', map);
    ...
    it('should return the actual value when looking up on a map that has ' +
       'a key and value but not that key', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
  ...
  describe('#insert', function() {
    ...
    it('inserts a value into a map that has values but not at the key that is ' +
       'being inserted and can read it out', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
    ...
    it('inserts a value into a map that has values at the key that is ' +
       'being inserted and can read it out', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
  ...
  describe('#remove', function() {
    ...
    it('deletes a value from a non-empty map that does not already have that key ' +
       'and can cannot read it out', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
    ...
    it('deletes a value from a non-empty map that does not already have that key ' +
       'and can read the other key out', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
      const clearedMap = Map.remove('anotherKey', map);
      const actualValue = Map.get('key', clearedMap);
    ...
    it('deletes a value from a non-empty map that does already have that key ' +
       'and can not read that key out', function() {
      const map:Map.Map<string, string> = Map.Map<string,string>([['key', 'value']]);
      const clearedMap = Map.remove('key', map);
      const actualValue = Map.get('key', clearedMap);
  ...
  describe('#containsKey', function() {
    ...
    it('should return true when the key is present in the map', function() {
      const map:Map.Map<string, number> = Map.Map<string, number>([['something', 4]]);
      const actualValue = Map.containsKey('something', map);
  ...
  describe('#foldl', function() {
    it('returns the initial value when given an empty map', function() {
      const map:Map.Map<string, number> = Map.Empty<string, number>();
      const actualValue:Map.Map<string, number> = Map.foldl(function(soFar:Map.Map<string, number>, key:string, num:number) {
        return Map.insert(key, num * 2, soFar);
      }, Map.Empty<string, number>(), map);
    ...
    it('returns the folded value when given a non empty map', function() {
      ...
      const expectedValue = Map.insert('something', 8, Map.Empty<string, number>());

      expect(actualValue).toEqualJSON(expectedValue);
    });

    it('returns the folded value when given a larger map', function() {
      const map:Map.Map<string, number> = Map.Map<string, number>([['something', 4], ['else', 8]]);
      const actualValue:Map.Map<string, number> = Map.foldl(function(soFar:Map.Map<string, number>, key:string, num:number) {
        return Map.insert(key, num * 2, soFar);
      }, Map.Empty<string, number>(), map);

      const expectedValue1 = Map.insert('something', 8, Map.Empty<string, number>());
